...
const DEFAULT_PROVIDER_RATE_LIMITS: Record<Provider, ProviderRateLimit> = {
  ...
  kie: { concurrency: 2, startIntervalMs: 1200 },
};
...
Options:
  ...
  --provider google|openai|dashscope|replicate|kie  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
export function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "replicate" && v !== "kie") {
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, replicate: null, kie: null };
        ...
        currentKey === "provider_limits" &&
        indent >= 4 &&
        (key === "google" || key === "openai" || key === "dashscope" || key === "replicate" || key === "kie")
      ) {
        config.batch ??= {};
        config.batch.provider_limits ??= {};
...
export async function loadExtendConfig(): Promise<Partial<ExtendConfig>> {
  const home = homedir();
  const cwd = process.cwd();
...
function getConfiguredProviderRateLimits(
  ...
  const configured: Record<Provider, ProviderRateLimit> = {
    ...
    kie: { ...DEFAULT_PROVIDER_RATE_LIMITS.kie },
  };

  for (const provider of ["replicate", "google", "openai", "dashscope", "kie"] as Provider[]) {
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, REPLICATE_API_TOKEN, or KIE_API_KEY.\n" +
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  if (provider === "google") return (await import("./providers/google.js")) as ProviderModule;
  if (provider === "dashscope") return (await import("./providers/dashscope.js")) as ProviderModule;
  if (provider === "replicate") return (await import("./providers/replicate.js")) as ProviderModule;
  if (provider === "kie") return (await import("./providers/kie.js")) as ProviderModule;
  return (await import("./providers/openai.js")) as ProviderModule;
}
...
function getModelForProvider(
  ...
  if (extendConfig.default_model) {
    ...
    if (provider === "kie" && extendConfig.default_model.kie) return extendConfig.default_model.kie;
  }
...
export async function generatePreparedTask(task: PreparedTask): Promise<TaskResult> {
...
export function createCliArgs(input: GenerationInput): CliArgs {
  return {
...
export async function loadGenerationConfig(): Promise<Partial<ExtendConfig>> {
  await loadEnv();
  return loadExtendConfig();
}

export async function generateImageTask(input: GenerationInput): Promise<TaskResult> {
  const extendConfig = await loadGenerationConfig();
  const args = mergeConfig(createCliArgs(input), extendConfig);
  if (!args.quality) args.quality = "2k";
  const task = await prepareSingleTask(args, extendConfig);
  return generatePreparedTask(task);
}

export async function runSingleMode(args: CliArgs, extendConfig: Partial<ExtendConfig>): Promise<void> {
  const task = await prepareSingleTask(args, extendConfig);
...
export async function runBatchMode(args: CliArgs, extendConfig: Partial<ExtendConfig>): Promise<void> {
