...
class TaskRepository:
    ...
    def list_unresolved_tasks(self, limit: int = 20) -> list[dict[str, Any]]:
        with self._connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT t.task_id, t.submission_id, t.type, t.state, t.created_at
                    FROM wf003_tasks t
                    INNER JOIN wf003_submissions s
                      ON s.submission_id = t.submission_id
                    WHERE s.finalized = 0
                      AND s.finalize_in_progress = 0
                      AND t.state NOT IN ('success', 'failed', 'error')
                      AND (t.result_url IS NULL OR t.result_url = '')
